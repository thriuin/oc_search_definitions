...
def filter_csv_record(csv_record,search: Search, fields: dict, codes: dict, format: str):
    if csv_record['fiscal_yr'] == '2019-2020':
        targets = csv_record['external_internal'].split(',')
        filtered_targets = [target for target in targets if target in ['extern', 'enterprise']]
        csv_record['external_internal'] = ','.join(filtered_targets)
        return True, csv_record
    return False, csv_record
